...
fun main(args: Array<String>) {
    ...
    var lastPosition = Point(0, 0)

    var shortestPath: List<Point>? = null
    var shortestPathMap: MutableMap<Point, Point> = mutableMapOf()
    var pathPointer = 0
    // game loop
    while (true) {
        val currentPosition = Point(input.nextInt(), input.nextInt())
        val nextPoint = Point(input.nextInt(), input.nextInt())
        path.add(nextPoint)
        if (path.size > 1 && path.first() == nextPoint && lastPoint != nextPoint) {
            pathComplete = true
        }
        if (pathComplete && path.first() == nextPoint && lastPoint != nextPoint) {
            lap++
        }

        System.err.println("Path complete: $pathComplete")
        System.err.println("LAP: $lap")
        ...
        if (lap >= 3 && nextPoint == path.last() && abs(getAngleOfNextTarget(nextCheckpointDist, 400)) <= 3) {
            println("${shortestPathMap[nextPoint]!!.x} ${shortestPathMap[nextPoint]!!.y} BOOST")
        } else if (lap >= 2) {
            if (shortestPath == null) {
                shortestPath = shortestPath(path.toList(), 400)
                System.err.println(shortestPath.size)
            ...
            thurst =
                getThrust(
                    distance(currentPosition, shortestPathMap[nextPoint]!!).roundToInt(),
                    getAngleOfNextTarget(nextCheckpointDist, 400)
                )
            println("${shortestPathMap[nextPoint]!!.x} ${shortestPathMap[nextPoint]!!.y} $thurst")
        } else {
            if (nextCheckpointAngle > 90 || nextCheckpointAngle < -90) {
                thurst = 0
            } else thurst = getThrust(nextCheckpointDist, nextCheckpointAngle)

            println("${nextPoint.x} ${nextPoint.y} $thurst")
        }
        lastPoint = nextPoint
